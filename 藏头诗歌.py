...
def predict(test_source_text=None, head=None):
    ...
    de_input = tf.constant(
        [[tail_tokenizer.word_index['<start>'], tail_tokenizer.word_index[head]]], dtype=tf.int64)
    ...
    while True:
        ...
        if out_words[-1] == '<end>' or len(out_words) >= 14:
            break
    next_line = ''.join(out_words[:-1])
    pre_line = test_source_text.split(',')
    if not pre_line[1]:
        return next_line
    return  pre_line[1] + ';' + next_line
...
for i in range(len(topics)):
    topic = topics[i] + ','
    if i == 0:
        next_poem_lines = predict(topic, topics[i])
    else:
        next_poem_lines = predict(topic + next_poem_lines, topic)
print(next_poem_lines)